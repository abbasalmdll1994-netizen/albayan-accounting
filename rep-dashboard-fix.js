...
function activateRepresentativeSection(){const section=new URLSearchParams(location.search).get('section');if(!section)return;let tries=0;const open=()=>{const b=Array.from(document.querySelectorAll('[data-section]')).find(x=>x.dataset.section===section);if(b){b.click();return}if(++tries<40)setTimeout(open,100)};open();}
