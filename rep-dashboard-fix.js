...
function bindDashboard(){
 ...
 document.addEventListener('click',e=>{
  ...
  if(b.dataset.paymentFlow==='1'){go('representative.html?section=debts&payment=1');return;}
  if(b.dataset.openSection){go('representative.html?section='+encodeURIComponent(b.dataset.openSection));return;}
  if(b.dataset.open==='closeFrame'){go('rep-daily-close.html');return;}
  if(b.dataset.open==='cashFrame'){go('cash-handover.html?mode=rep');return;}
  if(b.dataset.open==='repFrame'){go('representative.html?section=daily');return;}
  if(b.dataset.open==='salesFrame'){go('index.html?page=sales');return;}
 },true);
}
function activateRepresentativeSection(){const q=new URLSearchParams(location.search),section=q.get('section');if(!section)return;let tries=0;const open=()=>{const b=Array.from(document.querySelectorAll('[data-section]')).find(x=>x.dataset.section===section);if(b){b.click();return}if(++tries<80)setTimeout(open,100)};open();}
