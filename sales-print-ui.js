...
function install(){
 ...
 invoiceHTML=function(v){
  ...
  const discount=Math.max(0,Number(v.discount??v.loading)||0);
  ...
  const rows=[
   ...
   '<p><span>الخصم</span><b>'+money(discount)+'</b></p>',
   '<p><span>المدفوع</span><b>'+money(paid)+'</b></p>',
   '<p><span>المتبقي</span><b>'+money(remaining)+'</b></p>',
   '<p class="grand"><span>المجموع الكلي</span><span>'+money(total)+'</span></p>'
  ];
