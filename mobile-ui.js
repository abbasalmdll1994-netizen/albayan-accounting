...
function addBottom(){if(document.querySelector('.mobile-bottom'))return;const bar=document.createElement('div');bar.className='mobile-bottom no-print';bar.innerHTML='<button data-mob="home"><b>⌂</b>الرئيسية</button><button data-mob="purchaseOrders"><b>▣</b>المشتريات</button><button data-mob="sales"><b>▤</b>المبيعات</button><button data-mob="accounts"><b>◉</b>الحسابات</button><button data-mob="inventory"><b>□</b>المخزون</button>';document.body.appendChild(bar);bar.addEventListener('click',e=>{const b=e.target.closest('[data-mob]');if(!b)return;const target=document.querySelector('nav button[data-page="'+b.dataset.mob+'"]');if(target)target.click();setActive(b.dataset.mob)});setActive('home')}
