...
function install(){
 ...
 function apply(){const total=net();if(method.value==='cash')paid.value=String(total);else if(method.value==='credit')paid.value='0';paid.readOnly=method.value!=='partial';if(due){due.required=method.value!=='cash';due.closest('label')?.classList.toggle('payment-due-required',method.value!=='cash')}if(typeof updateSaleTotals==='function')updateSaleTotals()}
 ...
 form.addEventListener('submit',e=>{if(method.value!=='cash'&&due&&!due.value){e.preventDefault();e.stopImmediatePropagation();if(typeof notify==='function')notify('حدد تاريخ استحقاق الدين قبل حفظ الفاتورة.');due.focus()}},true);
 document.addEventListener('click',e=>{const b=e.target.closest('[data-edit-sale]');if(!b)return;setTimeout(()=>{const v=typeof state!=='undefined'?state.invoices.find(x=>x.id===b.dataset.editSale):null;if(!v)return;const total=typeof invoiceTotal==='function'?invoiceTotal(v):0;const p=Number(v.paid)||0;method.value=p<=0?'credit':cents(p)>=cents(total)?'cash':'partial';paid.readOnly=method.value!=='partial';if(due)due.required=method.value!=='cash'},0)});
